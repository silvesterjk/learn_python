...
def predict(system_msg, inputs, top_p, temperature, chat_counter, chatbot=[], history=[]):  
    headers = {
        ...
        "Authorization": f"Bearer {openai_gpt4_key}"  # Using the API key set in the code
    }
    ...
    with gr.Column(elem_id = "col_container"):
        ...
        with gr.Row():
            ...
            with gr.Accordion(label="System message:", open=False):
                # Replace the Textbox with a Dropdown for system_msg
                system_msg = gr.Dropdown(label="Instruct the AI Assistant to set its behavior", choices=["AI programming assistant", "ComedianGPT", "ChefGPT", "FitnessGuruGPT", ...], value="", info=system_msg_info)  # Fill in all the choices
